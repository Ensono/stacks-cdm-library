# PowerShell script to extract Ping Directory Service version and check if upgrade is needed
...
$kubectlCommand = "kubectl exec -it ds-cts-0 -n forgerock-dev -c ds -- /opt/opendj/bin/status --bindDN uid=admin --bindPassword:file /var/run/secrets/admin/dirmanager.pw -V"
